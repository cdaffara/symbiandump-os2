...
// This file produces 9 "unreferenced formal parameter" warnings on MS and CW
#if defined(__VC32__)
#pragma warning ( disable : 4100 )

#if !defined(_DEBUG)
// This file produces 1 "unreachable code" warning on MS for UREL builds
...
#define XML_T(x) (wchar_t)x
#define XML_L(x) L ## x
#else
#define XML_T(x) (unsigned short)x
#define XML_L(x) x
#endif
...
#ifndef SYMBIAN_MIN_SIZE
static Processor externalEntityInitProcessor;
static Processor externalEntityInitProcessor2;
static Processor externalEntityInitProcessor3;
#endif // SYMBIAN_MIN_SIZE
...
#define MALLOC(s) ( (parser)->m_mem.malloc_fcn ( ((parser)->m_mem.allocData), (s) ) )
#define REALLOC(p,s) ( (parser)->m_mem.realloc_fcn ( ((parser)->m_mem.allocData), (p), (s) ) )
#define FREE(p) ( (parser)->m_mem.free_fcn ( ((parser)->m_mem.allocData), (p) ) )
...
inline void* internal_malloc(void* allocData, size_t size)
{ return malloc(size); }

inline void* internal_realloc(void* allocData, void* ptr, size_t size)
{ return realloc(ptr, size); }

inline void internal_free(void* allocData, void* ptr)
{ free(ptr); }
...
XML_ParserCreate_MM(const XML_Char *encodingName,
  ...
  if (memsuite) {
    ...
    parser = (XML_Parser)memsuite->malloc_fcn(memsuite->allocData, sizeof(struct XML_ParserStruct));
    if (parser != NULL) {
      mtemp = &(parser->m_mem);
      mtemp->malloc_fcn = memsuite->malloc_fcn;
      mtemp->realloc_fcn = memsuite->realloc_fcn;
      mtemp->free_fcn = memsuite->free_fcn;
	  mtemp->allocData = memsuite->allocData;

    }
  }
  else {
    XML_Memory_Handling_Suite *mtemp;
    parser = (XML_Parser)internal_malloc(0, sizeof(struct XML_ParserStruct));
    if (parser != NULL) {
      mtemp = &(parser->m_mem);
      mtemp->malloc_fcn = internal_malloc;
      mtemp->realloc_fcn = internal_realloc;
      mtemp->free_fcn = internal_free;
	  mtemp->allocData = 0;
    }
  }
  ...
  if (len > bufferLim - bufferEnd) {
    ...
    if (neededSize  <= bufferLim - buffer) {
      ...
      int bufferSize = bufferLim - bufferPtr;
        // If the XML file is Very Big and corrupt,then bufferSize would increase drastically 
      //as the bufferLim incrases and BufferPtr doesnt move.Hence To Avoid XML_ERROR_NO_MEMORY
      //the bufferPtr is set to bufferEnd and bufferSize is controlled/Reset.The UpperLimit for bufferSize for
      // Wrong Parsed buffer has been set to 8KB.
    ...
    case XML_TOK_ENTITY_REF:
      {
        ...
        */
        ...
        if (entity->textPtr) {
          ...
          if (!context)
            return XML_ERROR_NO_MEMORY;
          if (!externalEntityRefHandler(handlerArg, name))
          ...
          BINDING **bindingsPtr)
{
  ...
  /* SYMBIAN: the following lines have been modified to correct a "Directly dereferenced 
     pointer 'tagNamePtr' Coverity error.
  */
  if (tagNamePtr) {
  ...
  if (nPrefixes) {
    ...
    for (; i < attIndex; i += 2) {
      if (appAtts[i][-1] == 2) {
        ...
        /* SYMBIAN: the following lines have been added to correct a "Variable 'id' assigned
           to NULL return value from 'lookup' " error found by Coverity
        */
        if (id == NULL) {
          return XML_ERROR_NO_MEMORY;
        }
    ...
    case XML_ROLE_GROUP_OPEN:
      if (prologState.level >= groupSize) {
        if (groupSize) {
          ...
          groupConnector = temp;
          if (dtd.scaffIndex) {
            int *temp = (int *)REALLOC(dtd.scaffIndex, groupSize * sizeof(int));
            if (temp == NULL)
              return XML_ERROR_NO_MEMORY;
            dtd.scaffIndex = temp;
          }
                ...
                XML_Bool isId, const XML_Char *value, XML_Parser parser)
{
  ...
  if (type->nDefaultAtts == type->allocDefaultAtts) {
    if (type->allocDefaultAtts == 0) {
      ...
      temp = (DEFAULT_ATTRIBUTE *)REALLOC(type->defaultAtts, (count * sizeof(DEFAULT_ATTRIBUTE)));
      if (temp == NULL)
        return 0;
      ...
      for (i = 0; name[i]; i++) {
        if (name[i] == XML_T(':')) {
          ...
          id->prefix = (PREFIX *)lookup(&dtd.prefixes, poolStart(&dtd.pool),
                                        sizeof(PREFIX));
          if (id->prefix && id->prefix->name == poolStart(&dtd.pool))
            poolFinish(&dtd.pool);
          else
            poolDiscard(&dtd.pool);
          break;
...
lookup(HASH_TABLE *table, KEY name, size_t createSize)
{
  ...
  if (table->size == 0) {
    ...
    table->v = (NAMED **)table->mem->malloc_fcn(table->mem->allocData, tsize);
    if (!table->v)
      return NULL;
    ...
    if (table->used == table->usedLim) {
      ...
      NAMED **newV = (NAMED **)table->mem->malloc_fcn(table->mem->allocData, tsize);
      if (!newV)
        return NULL;
      memset(newV, 0, tsize);
      ...
      table->mem->free_fcn(table->mem->allocData, table->v);
      table->v = newV;
      table->size = newSize;
  ...
  table->v[i] = (NAMED *)table->mem->malloc_fcn(table->mem->allocData, createSize);
  if (!table->v[i])
    return NULL;
...
hashTableClear(HASH_TABLE *table)
{
  ...
  for (i = 0; i < table->size; i++) {
    NAMED *p = table->v[i];
    if (p) {
      table->mem->free_fcn(table->mem->allocData, p);
      table->v[i] = NULL;
    }
  }
...
poolDestroy(STRING_POOL *pool)
{
  ...
  while (p) {
    BLOCK *tem = p->next;
    pool->mem->free_fcn(pool->mem->allocData, p);
    p = tem;
  }
  p = pool->freeBlocks;
  while (p) {
...
poolGrow(STRING_POOL *pool)
{
  ...
  if (pool->blocks && pool->start == pool->blocks->s) {
    ...
    pool->blocks = (BLOCK *)pool->mem->realloc_fcn(pool->mem->allocData, pool->blocks,
                                          offsetof(BLOCK, s)
    ...
    if (blockSize < INIT_BLOCK_SIZE)
      ...
      blockSize *= 2;
    tem = (BLOCK *)pool->mem->malloc_fcn(pool->mem->allocData, offsetof(BLOCK, s)
                                + blockSize * sizeof(XML_Char));
    if (!tem)
      return XML_FALSE;
    tem->size = blockSize;
