...
class CXmlEngContentHandler : public CBase, public Xml::MContentHandler
{

public:
	
	static CXmlEngContentHandler* NewL( CXmlEngDeserializer* aDeserializer,
									 MContentHandler* aContentHandler );
	
	~CXmlEngContentHandler();
...
*/
	CXmlEngContentHandler(CXmlEngDeserializer* aDeserializer, MContentHandler* aContentHandler);
...
*/
	TPtrC8 CidValue(const TDesC8& aCid);
...
*/	
	RTagInfo ConvertNameToLowerCaseL(const RTagInfo& aElement);
	
private: //data

/** Handle to deserializer **/
CXmlEngDeserializer* iDeserializer;

/** Handle to client's content handler **/
MContentHandler*		iContentHandler;

/** Indicates that xop:Include element is being processed **/
TUint iSkipIncludeElement;
TBool iSkipPrefixMapping;

/** Arrays for storing namespace mapping events **/
RPointerArray<HBufC8> iCachedPrefixes;
RPointerArray<HBufC8> iCachedUris;
RArray<TInt> iCachedErrors;
